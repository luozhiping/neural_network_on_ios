...
@available(iOS 10, *)
public class AveragePooling: SimpleCNNLayer {
    ...
    override public func createNetWork(device: MTLDevice) {
        cnn = MPSCNNPoolingAverage(device: device, kernelWidth: self.poolSize.0, kernelHeight: self.poolSize.1, strideInPixelsX: self.stride.0, strideInPixelsY: self.stride.1)
    }
